...
//ShadowShader
struct ShadowShader : IShader // Inheritate
{
	const Model& model;
	vec4 l;
	vec3 tri[3];
	vec4 norm[3];
	vec2 uv[3];
	vec4 frag_light_position[3];
	TGAImage diffusemap;
	TGAImage specularmap;
	TGAImage shadowmap;
	
	ShadowShader(const vec3 light, const Model& m, const vec3 eye) : model(m)
	{
		// 光线是一个方向向量，转换到观察空间直接乘 ModelView 即可
		l = normalized(ModelView * vec4{ light.x, light.y, light.z, 0. });
	}
	
	void shadow_map(const int width, const int height)
	{
	...
	virtual vec4 vertex(const int face, const int vert)
	{
		...
		//here also need it!!!!! because second loop won't use shadow_vertex()

		frag_light_position[vert] = Perspective * ModelViewLight * model.vert(face, vert);
	...
	virtual vec4 shadow_vertex(const  int  face, const  int vert)
	{
		//light vert position
		frag_light_position[vert] = Perspective * ModelViewLight * model.vert(face, vert);

		return frag_light_position[vert];
	}

	virtual vec4 shadow_bar(const vec3 shadow_bar) const
	{
		return vec4{ shadow_bar.x * frag_light_position[0] + shadow_bar.y * frag_light_position[1] + shadow_bar.z * frag_light_position[2] };
	}
...
int main(int argc, char** argv) {
	...
	init_perspective(0.1, 100 , 1, 1, 60.);                        // build the Perspective matrix
	...
	for (int m = 1; m < argc; m++)
	{
		...
		{
			Triangle clip = {
						  shader.shadow_vertex(f, 0),  // assemble the primitive
						  shader.shadow_vertex(f, 1),
						  shader.shadow_vertex(f, 2) };
			shadow(shadowmap, clip);
		}
	}
	for (int m = 1; m < argc; m++)
	{
