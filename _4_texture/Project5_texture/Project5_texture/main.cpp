...
struct PhongShader : IShader // Inheritate
{
    ...
    virtual vec4 normal(const int face, const int vert)
    {
        vec4 n = model.normal(face, vert);
        vec4 normal_direction = ModelView * vec4{ n.x,n.y,n.z,1. };
        nor[vert] = normal_direction.xyz();
        return Perspective * normal_direction;
    }

    virtual std::pair<bool, TGAColor> fragment(const vec3 bar,const vec3 normal) const
    {
        TGAColor gl_FragColor{ 255,255,255,255 };
        ...
        double spec = std::pow(std::max(r.z, 0.), 32);
...
int main(int argc, char** argv) {
    ...
    PhongShader shader({2,2,2},model);
        for (int f = 0; f < model.nfaces(); f++) {      // iterate through all facets
            ...
            Triangle clip = { shader.vertex(f, 0),  // assemble the primitive
                              shader.vertex(f, 1),
                              shader.vertex(f, 2) };
            rasterize(clip, normal, shader, framebuffer);   // rasterize the primitive
        }
